 ...
 */
inline fun setSuperLinearNoScoll(context: Context, recyclerView: SuperRecyclerView) {
    val layoutManager: LinearLayoutManager
    layoutManager = object : LinearLayoutManager(context) {
        override fun canScrollVertically(): Boolean {
            return false
 ...
 */
inline fun setLinearRecyclerView(context: Context, recyclerView: SuperRecyclerView) {
    val layoutManager = LinearLayoutManager(context)
    layoutManager.orientation = LinearLayoutManager.VERTICAL
    recyclerView.layoutManager = layoutManager
    recyclerView.itemAnimator = DefaultItemAnimator()
 ...
 * @param size 九宫格，一行的个数
 */
inline fun setGridRecyclerView(context: Context, recyclerView: SuperRecyclerView, size: Int) {
    val layoutManager = GridLayoutManager(context, size)
    recyclerView.layoutManager = layoutManager
    recyclerView.itemAnimator = DefaultItemAnimator()
    recyclerView.setRefreshProgressStyle(ProgressStyle.BallSpinFadeLoader)
 ...
 * @param size 九宫格，一行的个数
 */
inline fun setGridNoScroll(context: Context, recyclerView: SuperRecyclerView, size: Int) {
